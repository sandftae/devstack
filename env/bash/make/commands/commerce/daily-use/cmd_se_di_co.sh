...
cmd_se_di_co() {
  ...
  # execution
  if docker exec -it -u php "$php_container" sh -c "export TERM=xterm-256color && bin/magento setup:di:compile"; then
    printf "\n%b%b[✓] Compilation completed successfully.%b\n" "${COLOR_GREEN}" "${C_BOLD}" "${C_NC}"
    return 0
  fi

  printf "\n%b%b[!] ERROR%b: compilation failed\n" "${COLOR_RED}" "${C_BOLD}" "${C_NC}"
  return 1
}