#!/bin/bash
# ============================================================
# COMMAND: varnish_enable
# DESCRIPTION: enable/unsilence varnish container
# ============================================================

# cmd_varnish_enable function
cmd_varnish_enable() {
  local silence="false"
  ...
  # rebuild images
if spinner "${C_BOLD}Unsilencing${C_NC} Varnish" _docker_compose up -d --build "$ssl_service_name"; then
  printf "%b✔%b Varnish is now %b%s%b!\n" \
    "${COLOR_GREEN}" "${C_NC}" "${COLOR_YELLOW}" "enabled (unsilenced)" "${C_NC}"

      printf "\n%b%b[!] NOTE%b: also, enable varnish in %bStores > Configuration > Advanced > System > FPC > Caching Application%b.\n" \
    "${COLOR_YELLOW}" "${C_BOLD}" "${C_NC}" "${C_BOLD}" "${C_NC}"

  printf "%b%b[!] REMINDER%b: don't forget to run %bmake varnish-disable%b after testing.\n" \
    "${COLOR_YELLOW}" "${C_BOLD}" "${C_NC}" "${C_BOLD}" "${C_NC}"
