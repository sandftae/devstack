...
> This devstack is created specifically for **local development and testing**. It contains configurations designed for debugging, performance monitoring, and are **not secure** for public environments.
>
> **DO NOT use this tool for Production or Staging environments.**

> [!NOTE]
> ### Infrastructure vs. Application
> This tool builds the **"house"** (the servers, networking, services, and etc). You are responsible for bringing the **"furniture"**. It means cloning your source code, managing `auth.json` credentials, and executing application-level installs like `composer install` or `yarn install`, and etc.
...
1.  **Makefile (Base):** Minimalist. Only contains core commands to build, start, stop, and enter containers. It is for the devs who prefer running CLI commands directly inside the Docker container or by using docker commands.
2.  **Powermake:** Extended Makefile. Includes dozens of "power" commands for Adobe Commerce (cache, reindex, mode switching), metrics, docker, and folder management. It is for the devs who don't want to `exec` docker container and run command outside. Just keep in mind this boilerplate tries to cover as much daily-use command as possible, but I'm not sure if it did it, tbh = )

**Tip:** If you want more power, simply copy specific commands (or the whole file) from `Powermake` into your main `Makefile`.
...
1. **Clone the repository**
   ...
   # optional cleaning action, be sure you are deleting devstack`s git service files/folders and devstack`s *.md files  
...
> [!NOTE]
>
> **Defaults:** If you do not modify these, the devstack will use predefined values. You can return to this later, but keep im mind you will need to:
>  - run ``make rebuild-project``, this command will recreate all images/containers, without changing compose file


| Variable                         | Allowed Values                                                                     | Default Values              | Description / Note                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
|----------------------------------|:-----------------------------------------------------------------------------------|:----------------------------|:----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `ADOBE_COMMERCE_MODE`            | `developer`, `production`                                                          | `developer`                 | Sets the Adobe Commerce mode. If changed manually, you must rebuild Nginx.<br/>  Use: <br/> - `make mode-developer` to set ``developer mode`` <br/> - `make mode-production` to set ``production mode``<br/><br/>  Both command will automatically recreate ``nginx`` service, so no needs to do extra work                                                                                                                                                                 |
| `DEVELOPER_MODE_BYPASS_VARNISH`  | `"true"`, `"false"`                                                                | `"true"`                    | **``"true"``**: varnish ``is "bypassed"`` (direct upstream to ``nginx`` service) <br>  **``"false"``** : varnish ``is not "bypassed"`` and the is taken from cache<br/><br/>  You can set it for both Adobe Commerce modes ``production`` and ``developer``,<br/>  but keep in mind it will lead you to very strange cache results for ``developer mode``.<br/><br/> **Please, use it only with the**  ``production mode`` **to test cache results and cache-related bugs** |
| `NODE_VERSION`                   | `16-alpine`, `17-alpine`,<br/> `18-alpine`, `20-alpine`,<br/> `{N}-alpine`         | `20-alpine`                 | Defines the ``node.js`` version for the `web-app` container                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `NODE_OPTIONS_OPEN_SSL`          | `--openssl-legacy-provider`<br/>  or *empty*, means just``NODE_OPTIONS_OPEN_SSL=`` | `--openssl-legacy-provider` | **REQUIRED** for ``node >= 17`` (currently, it is default settings).  Keep empty if you downgrade to ``node < 17``                                                                                                                                                                                                                                                                                                                                                          |
| `XDEBUG_ENABLED`                 | `true`, `false`                                                                    | `true`                      | Enables or disables the Xdebug extension in the `php-app` container. It is enabled by default.                                                                                                                                                                                                                                                                                                                                                                              |


3. **Launch the DevStack GUI inside the ``<your-project-root-folder>``**
    ```bash
   make magma-build 
   ```
   > [!NOTE]
...
3. **Frontend (Headless / PWA)**
    ...
    make web-app 
   ```
    - clone your frontend project (Vue Storefront, PWA Studio, etc.)
    - run your installation commands manually (e.g., ``npm install`` or ``yarn install``)


4. **Node Version and Version upgrade/downgrade**
...
- change it from this ``NODE_VERSION=20-alpine`` to this ``NODE_VERSION=16-alpine``, ``NODE_VERSION=17-alpine``, etc.
...
```bash
  ...
  make up
```
**This actions will build new node image and up env with new nodejs docker container**

> [!NOTE]
>
> Depending on the use case and the reason for changing the version, you may need to delete the ``web-app`` image for the corresponding node.js version and rebuild it.
...
> [!TIP]
>
> **The Service Dashboard** is your "Command Center." It contains direct links to all active service GUIs (like Mailpit, phpMyAdmin, RabbitMQ, etc.) so you don't have to remember every port number.

**The Service Dashboard** view:

<img src="docs/media/github/service_dashboard.png" width="250" height="125" alt="Service Dashboard"/>
<img src="docs/media/github/service_credentials.png" width="250" height="125" alt="Service Credentials"/>
